...
#define PTR_JPGC   pJpegDecCont
    ...
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_DEC_OUT_DIS, 0);        //wjm test
    ...
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_JPEG_FILRIGHT_E, PTR_JPGC->info.fillRight);
    ...
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_JPEG_SLICE_H, //1);    //wjm test
                   PTR_JPGC->info.sliceHeight);
...
#define JPG_STR     pJpegDecCont->stream
    ...
    if(!VPUMemJudgeIommu()){
        SetDecRegister(PTR_JPGC->jpegRegs, HWIF_RLC_VLC_BASE, (RK_U32)JPG_STR.streamBus + addrTmp);
    }else{
        SetDecRegister(PTR_JPGC->jpegRegs, HWIF_RLC_VLC_BASE, (RK_U32)JPG_STR.streamBus | (addrTmp << 10));
    }
    ...
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_JPEG_STREAM_ALL,    //0);
                   PTR_JPGC->info.streamEnd);
...
#define PTR_JPGC   pJpegDecCont
       ...
       (PTR_JPGC->ppInstance != NULL && !PTR_JPGC->ppControl.usePipeline))
    {
        if(PTR_JPGC->info.givenOutLuma.vir_addr == NULL)
        {
            ...
            /*tmp =
                VPUMallocLinear(&(PTR_JPGC->asicBuff.outLumaBuffer), (PTR_JPGC->image.sizeLuma + (PTR_JPGC->image.sizeChroma)));
            if(tmp == -1)
            {
                return (JPEGDEC_MEMFAIL);
            }*/
            ...
            if(PTR_JPGC->image.sizeChroma)
            {
                PTR_JPGC->asicBuff.outChromaBuffer.phy_addr = PTR_JPGC->asicBuff.outLumaBuffer.phy_addr + PTR_JPGC->image.sizeLuma;

                if(PTR_JPGC->info.operationType != JPEGDEC_BASELINE)
                {
                    PTR_JPGC->asicBuff.outChromaBuffer2.phy_addr = PTR_JPGC->asicBuff.outChromaBuffer.phy_addr + (PTR_JPGC->image.sizeChroma/2);                
                }else{
        ...
        JPEGDEC_TRACE_INTERNAL(("ALLOCATE: Luma virtual %x bus %x\n",
                                (RK_U32) PTR_JPGC->asicBuff.outLumaBuffer.vir_addr,
                                PTR_JPGC->asicBuff.outLumaBuffer.phy_addr));
...
void JpegFlushRegs(JpegDecContainer * pJpegDecCont)
{
    ...
    /*{   
        int i;
        //memset(&pJpegDecCont->jpegRegs[60], 0, 40*4);
        for(i=0;i<60;i++)
        {
            ALOGE("before reg[%d]=0x%x\n",i,pJpegDecCont->jpegRegs[i]);
        }
    }*/
...
int SetPostProcessor(unsigned int * reg,VPUMemLinear_t *dst,int inWidth,int inHeigth,
								int outWidth,int outHeight,int inColor, PostProcessInfo *ppInfo, unsigned int out_phyaddr)//,int outColor,HW_BOOL dither)
{
...
#define PP_OUT_FORMAT_YUV420INTERLAVE    5
	...
	if(out_phyaddr)
		SetDecRegister(reg, HWIF_PP_OUT_LU_BASE, out_phyaddr);
	else
		...
		case	PP_OUT_FORMAT_YUV420INTERLAVE:
            {
                RK_U32 phy_addr = 0;
                if(out_phyaddr){
                    phy_addr = out_phyaddr;
    			}
    			else{
                    phy_addr = dst->phy_addr;
    			}
                if(!VPUMemJudgeIommu()){
                    SetDecRegister(reg, HWIF_PP_OUT_CH_BASE, (phy_addr+outWidth*outHeight));
                }else{
                    if(outWidth*outHeight > 0x400000){
                        ALOGE("out offset big then 22bit iommu map may be error");
                    }else{
                        SetDecRegister(reg, HWIF_PP_OUT_CH_BASE, (phy_addr | ((outWidth*outHeight) << 10)));
                    }
                }
