...
------------------------------------------------------------------------------*/
RK_S32 JPEG_EncAsicControllerInit(asicData_s * asic)
{
    ASSERT(asic != NULL);
    ...
    /* get ASIC ID value */
    asic->regs.asicHwId = JPEG_EncAsicGetId(asic->ewl);
...
------------------------------------------------------------------------------*/
void JPEG_EncAsicSetQuantTable(asicData_s * asic,
...
/*------------------------------------------------------------------------------
------------------------------------------------------------------------------*/
RK_U32 JPEG_EncAsicGetId(const void *ewl)
{
    return 0x82701110;
}
...
------------------------------------------------------------------------------*/
void JPEG_EncAsicRecycleInternalImage(regValues_s * val)
{
...
/*------------------------------------------------------------------------------
------------------------------------------------------------------------------*/
void JPEG_CheckRegisterValues(regValues_s * val)
{
    RK_U32 i;
...
//	LOGD("val->regMirror[24] 0x%x \n",val->regMirror[24]);
    ...
    JPEG_EncFlushRegs(val);

}

void JPEG_EncFlushRegs(regValues_s * val)
{
	int i;

	/*for(i=0 ; i< ENC8270_REGISTERS;i++)
	{
		printf("EncAsic: REGISTER %d VALUE 0x%08x\n",i,
							val->regMirror[i]);
	}*/

    if (VPUClientSendReg(val->socket, val->regMirror, val->register_size))
        ALOGD("EncFlushRegs fail\n");
    else
        ;//LOGV("EncFlushRegs success\n");
...
/*------------------------------------------------------------------------------
------------------------------------------------------------------------------*/
void JPEG_EncAsicGetRegisters(const void *ewl, regValues_s * val)
{
    RK_U32 word;
...
/*------------------------------------------------------------------------------
------------------------------------------------------------------------------*/
void JPEG_EncAsicStop(const void *ewl)
{
...
/*------------------------------------------------------------------------------
------------------------------------------------------------------------------*/
RK_U32 JPEG_EncAsicGetStatus(regValues_s *val)
{
