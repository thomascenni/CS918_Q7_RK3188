...
#define RK29_JPEG_ENCODER_REGISTER_NUM 96
#define RK30_JPEG_ENCODER_REGISTER_NUM 164
...
JpegEncRet JpegInit(const JpegEncCfg * pEncCfg, jpegInstance_s ** instAddr)
{
    ...
    *instAddr = NULL;
    ...
    JPEG_EncAsicSetQuantTable(&inst->asic, inst->jpeg.qTable.pQlumi,
    ...
    inst->asic.regs.socket = VPUClientInit(VPU_ENC);
	{
		...
		if (VPUClientGetHwCfg(inst->asic.regs.socket, (RK_U32*)&hwCfg, sizeof(hwCfg)))
	    {
	        VPUClientRelease(inst->asic.regs.socket);
	        return -1;
	    }
		if(hwCfg.reg_size == (164*4))
			inst->asic.regs.register_size  = RK30_JPEG_ENCODER_REGISTER_NUM;
		else
			inst->asic.regs.register_size = RK29_JPEG_ENCODER_REGISTER_NUM;
	}
    (void) JPEG_EncAsicControllerInit(&inst->asic);
...
void JpegShutdown(jpegInstance_s * data)
{
    ...
    JPEG_EncAsicMemFree_V2(&data->asic);
