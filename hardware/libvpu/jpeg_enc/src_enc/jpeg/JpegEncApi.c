...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    JPEG_EncSetColorConversion(&pEncInst->preProcess, &pEncInst->asic);
...
JpegEncRet JpegEncSetPictureSize(JpegEncInst inst, const JpegEncCfg * pEncCfg)
{
    ...
    /* Check that configuration is valid */
    if(JPEG_EncPreProcessCheck(&pEncInst->preProcess) == ENCHW_NOK)
    {
        APITRACE
    ...
    if(JPEG_EncAsicMemAlloc_V2(&pEncInst->asic, (RK_U32) pEncInst->jpeg.width,
                         ...
                         JpegEncOut * pEncOut/*, VPUMemLinear_t outMem*/)
{
    ...
    if(jpeg->codingType == ENC_WHOLE_FRAME)
    {
        ...
        JPEG_EncPreProcess(asic, preProcess);
    }
    else
    {
