...
deinterlace_dev::deinterlace_dev(void *vpu_mem_ctx)
    :dev_status(USING_NULL),dev_fd(-1),mPool(NULL),priv_data(NULL)
{
...
status_t deinterlace_dev::perform(VPU_FRAME *frm, uint32_t bypass)
{
    ...
    vpu_display_mem_pool* pool = (vpu_display_mem_pool*)mPool;
    if( (frm->FrameHeight*frm->FrameWidth*3/2) <= pool->buff_size ){
        ret = VPUMallocLinearFromRender(&deInterlaceFrame, pool->buff_size,mPool);
    if (!ret) {
        ...
        if (!VPUMemJudgeIommu()) {
            srcCAddr = frm->FrameBusAddr[1];
            dstCAddr = deInterlaceFrame.phy_addr + width*height;
        } else {
            srcCAddr = frm->FrameBusAddr[0] + ((width*height)<<10);
            dstCAddr = deInterlaceFrame.phy_addr + ((width*height)<<10);
        }
        ...
        ALOGE("mem pool size is no big enought");
        ret = BAD_VALUE;
    }
    return ret;
}
