...
int Rk_AvcEncoder::pv_rkavcencoder_init(AVCEncParams1 *aEncOption, uint8_t* aOutBuffer,uint32_t* aOutputLength)
{
...
int Rk_AvcEncoder::pv_rkavcencoder_oneframe(uint8_t* aOutBuffer, uint32_t* aOutputLength,
    ...
    if(1)
    {
...

        {
           ...
           if (!VPUMemJudgeIommu()) {
               ...
               encIn.busChromaU = encIn.busLuma | ((w * cml->lumHeightSrc) << 10);
               encIn.busChromaV = encIn.busChromaU + ((((w + 1) >> 1) * ((cml->lumHeightSrc + 1) >> 1)) << 10);
           }
        }
#else
        {
...
void Rk_AvcEncoder::H264encSetInputAddr(unsigned long input)
{
       commandLine_s *cml = &cmdl;
	u32 w = (cml->lumWidthSrc + 15) & (~0x0f);
	...
	encIn.busLuma = input;
        if (!VPUMemJudgeIommu()) {
            ...
            encIn.busChromaU= input | ((w * cml->lumHeightSrc) << 10);
            encIn.busChromaV = encIn.busChromaU + ((((w + 1) >> 1) * ((cml->lumHeightSrc + 1) >> 1)) << 10);
        }
	return;
}
void Rk_AvcEncoder::H264encSetintraPeriodCnt()
{
       intraPeriodCnt = cmdl.intraPicRate;
	return;
}
void Rk_AvcEncoder::h264encSetmbperslice(int line_per_slice)
{
...
int Rk_AvcEncoder::H264encSetInputFormat(H264EncPictureType inputFormat)
{
	H264EncPreProcessingCfg preProcCfg;
	H264EncRet ret;
	H264EncInst encoder = (H264EncInst)&h264encInst;
...
void Rk_AvcEncoder::pv_rkavcencoder_getconfig(AVCEncParams1 *vpug)
{
	h264Instance_s *pEncInst = &h264encInst;
...
ALOGV("pv_rkavcencoder_getconfig bitrate w %d h %d %d rc %d fps %d cabac_flag %d idc %d",
		vpug->width,vpug->height,vpug->bitRate,vpug->rc_mode,vpug->framerate,vpug->enableCabac,vpug->cabacInitIdc);
}
void Rk_AvcEncoder::pv_rkavcencoder_setconfig(AVCEncParams1 *vpug)
{
    h264Instance_s *pEncInst = &h264encInst;
    u32 testid = 0;
...
ALOGV("pv_rkavcencoder_setconfig bitrate w %d h %d bitrate %d rc %d fps %d  qphdr %d qpmax %d qpmin %d intraqp %d ret %d fixintraqp %d testid %d vpug->enablaCabac %d",
...
int Rk_AvcEncoder::h264encFramerateScale(int inframerate, int outframerate)
{
	int curframenum, ret=1;
	if(outframerate<=0 || outframerate>30)
		return 0;
...
int Rk_AvcEncoder::pv_rkavcencoder_deinit()
{
	H264EncRet ret;
    commandLine_s * cml = &cmdl;
    H264EncInst encoder = (H264EncInst)&h264encInst;
...
extern "C"
void *  get_class_RkAvcEncoder(void)
{
    return (void*)new Rk_AvcEncoder();
}

extern "C"
void  destroy_class_RkAvcEncoder(void * AvcEncoder)
{
    delete (Rk_AvcEncoder *)AvcEncoder;
    AvcEncoder = NULL;
}
...
extern "C"
int init_class_RkAvcEncoder(void * AvcEncoder, EncParams1 *aEncOption, uint8_t* aOutBuffer,uint32_t* aOutputLength)
{
	Rk_AvcEncoder * Avcenc = (Rk_AvcEncoder *)AvcEncoder;
	return Avcenc->pv_rkavcencoder_init((AVCEncParams1 *)aEncOption, aOutBuffer, aOutputLength);
}

extern "C"
int deinit_class_RkAvcEncoder(void * AvcEncoder)
{
	Rk_AvcEncoder * Avcenc = (Rk_AvcEncoder *)AvcEncoder;
	return Avcenc->pv_rkavcencoder_deinit();
}

extern "C"
int enc_oneframe_class_RkAvcEncoder(void * AvcEncoder, uint8_t* aOutBuffer, uint32_t* aOutputLength,
                                     uint8_t *aInBuffer,uint32_t aInBuffPhy,uint32_t* aInBufSize,uint32_t* aOutTimeStamp,bool*  aSyncFlag)
{
	int ret;
	...
	ret = Avcenc->pv_rkavcencoder_oneframe(aOutBuffer, aOutputLength,aInBuffer,
...
int set_idrframe_class_RkAvcEncoder(void * AvcEncoder)
{
...
int set_config_class_RkAvcEncoder(void * AvcEncoder,AVCEncParams1 *vpug)
{

	Rk_AvcEncoder * Avcenc = (Rk_AvcEncoder *)AvcEncoder;
	Avcenc->pv_rkavcencoder_setconfig(vpug);
	return 0;
}

extern "C"

int get_config_class_RkAvcEncoder(void * AvcEncoder,AVCEncParams1 *vpug)
{

	Rk_AvcEncoder * Avcenc = (Rk_AvcEncoder *)AvcEncoder;

	Avcenc->pv_rkavcencoder_getconfig(vpug);
	return 0;
}

extern "C"

int set_inputformat_class_RkAvcEncoder(void * AvcEncoder,H264EncPictureType inputFormat)
{
