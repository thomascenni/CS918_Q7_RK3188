...
#define		DEBUG_PRINT_BITS_H		printf//ALOGE//printf

/*
extern u32 h264bsdDecodeCavlc(strmData_t * pStrmData,
                             seqParamSet_t * pSeqParamSet,
                             picParamSet_t * pPicParamSet,
                             sliceHeader_t * pSliceheadCavlc,
                             nalUnit_t * pNalUnit,
                             u32 numRefFrames,
                             int *skip_run_tmp,
                             u8 *non_zero_count,
                             i32 *skip_run_num,
                         	 i32 *frame_last_mb);
extern void h264dec_cavlc_add_Stream(strmData_t * pStrmData, u8 *strm, int strmstride,
										int *strmbitlen, int *skip_run_p, int skip_run_num);
extern void h264_cavlc_creat_stream(u8 *strm, int *strmbitlen, int skip_run, int num_refidx0, int num_refidx1,
							int st_bitoffset, int aft_bitoffset, int mbtype, int mvdxL0, int mvdyL0, int is_last_mb);
extern void h264_cavlc_creat_stream_activeNotZero(u8 *strm, int *strmbitlen, int skip_run, int num_active_idx0, int num_active_idx1,
							int st_bitoffset, int aft_bitoffset, int mbtype, int mvdxL0, int mvdyL0, int is_last_mb);
*/
...
u32 H264RunAsic(decContainer_t * pDecCont, DecAsicBuffers_t * pAsicBuff)
{
    ...
    if(pSliceHeader->fieldPicFlag && pSliceHeader->bottomFieldFlag)
    {
        if (!VPUMemJudgeIommu()) {
            ...
            if (pAsicBuff->outPhyAddr >= 1024 || pSps->picWidthInMbs * 16 >= 0x400000) {
                ALOGE("%s %d, enable iommu, fatal error!!!\n", __func__, __LINE__);
                return -1;
            }
            SetDecRegister(pDecCont->h264Regs, HWIF_DEC_OUT_BASE,
                           pAsicBuff->outPhyAddr | (pSps->picWidthInMbs * 16) << 10);
        }
    }
    else
    {
...

    {
        ...
        if(pAsicBuff->enableDmvAndPoc && pDecCont->h264ProfileSupport != 2)
        {
            ...
            if (!VPUMemJudgeIommu()) {
                ...
                if (pAsicBuff->outPhyAddr >= 1024 || (dirMvOffset>>4) >= 0x400000) {
                    ALOGE("%s %d, enable iommu, fatal error!!!\n", __func__, __LINE__);
                    return -1;
                }
                SetDecRegister(pDecCont->h264Regs, HWIF_DIR_MV_BASE,
                               pAsicBuff->outPhyAddr | (dirMvOffset << 6));
            }
        }
...
/***************************************************************************/
	...
	for(run_hard_times=0;run_hard_times<3;run_hard_times++)	//for ts
	{
		H264FlushRegs(pDecCont);

#if H264_USE_PREALLOC_VPU_MEMORY
        storage_t *pStorage = &pDecCont->storage;
        /*
         ** pre alloc vpu memory for next frame. make malloc more effective.
        */
        VPUMemLinear_t* vpu_mem = NULL;
        if (!(pStorage->dpb->pre_alloc.allocated)) {
            vpu_mem = &pStorage->dpb->pre_alloc.vpu_mem;
            memset(vpu_mem, 0, sizeof(VPUMemLinear_t));
            if(VPUMallocLinear(vpu_mem, pStorage->dpb->picbuffsie)){
                ALOGE("VPUMallocLinear failed.");
                return X170_DEC_SYSTEM_ERROR;
            } else {
                pStorage->dpb->pre_alloc.allocated = 1;
            }
        }
#endif
	    {
		...
		seqParamSet_t * pSeqParamSet = pDecCont->storage.activeSps;
		picParamSet_t * pPicParamSet = pDecCont->storage.activePps;

		if(pSeqParamSet == NULL || pPicParamSet == NULL)
			break;
		if(pPicParamSet->entropyCodingModeFlag || pDecCont->storage.cavlc_fill_en ||
			(pSeqParamSet->picWidthInMbs*pSeqParamSet->picHeightInMbs <= 8192))
			break;

		if(((pDecCont->h264Regs[1]==0x4101)||(pDecCont->h264Regs[1]==0x10100)) && (run_hard_times==0))
		{
			#if SHOW_DECODE_TIME
			gettimeofday(&sum_tv1,NULL);
			#endif

			strmData_t *pStrmData = &pDecCont->storage.strm_cavlc;
			nalUnit_t *pNalUnit = &pDecCont->storage.nalUnit;
			int numRefFrames = pDecCont->storage.dpb->numRefFrames;
			int tmp, cavlc_decode = 0;
			printf("%s: skip cavlc...\n", __func__);
			memcpy(pDecCont->h264Regs, h264regtmp, sizeof(h264regtmp));
		}else if((pDecCont->h264Regs[1]==0x8100) && (run_hard_times==1))
		{
			memcpy(pDecCont->h264Regs, h264regtmp, sizeof(h264regtmp));
		}else
		{
...
//    pDispoutbuff->pDispbuffer->Isdisplay = 1;
...

    {
        ...
        u32 bytes_processed;
        const u32 start_address = (pDecCont->hwStreamStartBus & (~DEC_X170_ALIGN_MASK)) + pDecCont->hwStreamStartOffset;
...
static void H264FlushRegs(decContainer_t * pDecCont)
{
    ...
    if(pDecCont->h264ProfileSupport != H264_BASELINE_PROFILE)
    {
        SetDecRegister(pDecCont->h264Regs, HWIF_QTABLE_BASE,
                        pDecCont->asicBuff->cabacInit.phy_addr);
    }
...
void H264SetupVlcRegs(decContainer_t * pDecCont)
{
    ...
    for(i = 0; i < 16; i++)
    {
        if (pDpb->buffer[i]) {
            ...
            value = (diffPoc < itmp ? 0x1 : 0) | (pDpb->buffer[i]->isFieldPic ? 0x2 : 0);
            if(!VPUMemJudgeIommu()){
                pDecCont->asicBuff->refPicList[i] |= value;
            }else{
                pDecCont->asicBuff->refPicList[i] |= (value << 10);
            }
        }
    }
...
void H264InitRefPicList(decContainer_t * pDecCont)
{
    ...
    if(pSliceHeader->fieldPicFlag)
    {
        ...
        {
            i = 0;
            while (!isIdr &&  NULL != dpb->buffer[list0[i]] &&
                IS_REF_FRAME_F(dpb->buffer[list0[i]])) i++;
            list0[i] = 15;
        }
        /* list 1 */
    ...
    else
    {
        ...
        if (pDecCont->storage.view && !pDecCont->storage.nonInterViewRef)
        {
            i = 0;
            while (!isIdr &&
                    NULL != dpb->buffer[list0[i]] &&
                    IS_REF_FRAME(dpb->buffer[list0[i]])) i++;
            list0[i] = 15;
        }
        /* list 1 */
    ...
    if(pSliceHeader->fieldPicFlag)
    {
        ...
        {
            i = 0;
            while (!isIdr && NULL != dpb->buffer[listP[i]] &&
                IS_REF_FRAME_F(dpb->buffer[listP[i]])) i++;
            listP[i] = 15;
        }
        for(i = 0; i < 16; i++)
        {
    ...
    else
    {
        ...
        if (pDecCont->storage.view && !pDecCont->storage.nonInterViewRef)
        {
            i = 0;
            while (!isIdr &&
                    NULL != dpb->buffer[listP[i]] &&
                    IS_REF_FRAME(dpb->buffer[listP[i]])) i++;
            listP[i] = 15;
        }
        for(i = 0; i < 16; i++)
        {
...
void h264StreamPosUpdate(decContainer_t * pDecCont)
{
    ...
    tmp = (pDecCont->hwStreamStartOffset & DEC_X170_ALIGN_MASK) * 8;
    ...
    // invalidate operate when iommu enable.
	if (!VPUMemJudgeIommu()) {
        tmp = pDecCont->hwStreamStartBus + pDecCont->hwStreamStartOffset;   /* unaligned phy_base */
	}else{
	   if (pDecCont->hwStreamStartBus >= 1024 ||pDecCont->hwStreamStartOffset >= 0x400000) {
            ALOGE("%s %d, enable iommu, fatal error!!!\n", __func__, __LINE__);
            return;
       }
       tmp = pDecCont->hwStreamStartBus | (pDecCont->hwStreamStartOffset << 10);   /* unaligned phy_base */
	}
