...
/* Code */
    ...
    if(id == pStorage->activeViewSpsId[0] ||
...
/* Code */
    ...
    *accessUnitBoundaryFlag = HANTRO_FALSE;
    ...
    if ((storage->aub->prevFrameNum != frameNum) || (storage->aub->prevTimeStamp != tsLow))
    {
        storage->aub->prevFrameNum  = frameNum;
        storage->aub->prevTimeStamp = tsLow;
        *accessUnitBoundaryFlag = HANTRO_TRUE;
    } else {
        ALOGI("found skipRedundantSlices");
    }
...
u32 h264bsdAllocateSwResources(storage_t * pStorage, u32 isHighSupported, u32 ts_en)
{
    ...
    return h264bsdResetDpb(pStorage->dpb, pStorage->picSizeInMbs,
                          ...
                          pSps->maxFrameNum,
                          pSps->monoChrome, isHighSupported, ts_en, pStorage->immediate_flag);
}
}
