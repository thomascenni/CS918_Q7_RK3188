...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    pDecCont->refBufSupport = 0 ;//hwCfg.refBufSupport;
...
void H264DecRelease(decContainer_t *pDecCont)
{
    ...
    if (pStorage->dpb->pre_alloc.allocated) {
        ...
        pStorage->dpb->pre_alloc.allocated =0;
    }
#endif

    h264bsdFreeDpb(pDecCont->storage.dpbs[0]);
    if(pDecCont->storage.numViews){
        h264bsdFreeDpb(pDecCont->storage.dpbs[1]);
    }
                         ...
                         H264DecOutput * pOutput)
{
    ...
    pDecCont->hwStreamStartOffset = 0;
    ...
    do
    {
        ...
        default : {
            decResult = (RET_E)h264bsdDecode(pDecCont, tmpStream, strmLen, pInput->picId, &numReadBytes, pInput->TimeLow, pInput->TimeHigh);
        ...
        case H264BSD_HDRS_RDY: {
            ...
            if (!h264SpsSupported(pDecCont))
            {
                ...
                if (sps->picWidthInMbs > (4096>>4) || sps->picHeightInMbs > (2304>>4)) {
        ...
        case H264BSD_PIC_RDY: {
            ...
            pDecCont->hwStreamStartOffset += pInput->start_offset;
        ...
        case H264BSD_PARAM_SET_ERROR: {
            ...
            pDecCont->hwStreamStartBus = pInput->streamBusAddress;
            pDecCont->hwStreamStartOffset = (u32)(tmpStream - pInput->pStream);
    ...
    if (pDecCont->streamPosUpdated){
		...
		if((pDecCont->storage.sliceHeader[1].fieldPicFlag==0)){
            if(!pDecCont->storage.numViews){
			pOutput->dataLeft = 0;
            }
		}
...
void h264UpdateAfterPictureDecode(decContainer_t * pDecCont)
{
    ...
    if (pStorage->validSliceInAccessUnit)
    {
        ...
        if (pStorage->view == 0)
            ...
            if (pStorage->dpb->numOut > pStorage->lastBaseNumOut)
                h264DpbAdjStereoOutput(pStorage->dpbs[0], pStorage->dpb->numOut);
            else
    ...
    if (pStorage->validSliceInAccessUnit || pStorage->view)
...
u32 h264SpsSupported(const decContainer_t * pDecCont)
{
    ...
    if(sps->picWidthInMbs * 16 > pDecCont->maxDecPicWidth ||
       ...
       (sps->picWidthInMbs > (4096>>4) || sps->picHeightInMbs > (2304>>4)))
    {
...
H264DecRet H264DecSetMvc(decContainer_t *pDecCont)

{
    ...
    VPUClientGetHwCfg(pDecCont->socket, (RK_U32*)&hwCfg, sizeof(DWLHwConfig_t));
