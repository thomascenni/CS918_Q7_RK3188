...
/* Code */
    ...
    if (!pStorage->checkedAub)
    {
        tmp = h264bsdCheckAccessUnitBoundary(&strm,
                                             &nalUnit,
                                             pStorage, &accessUnitBoundaryFlag, tsLow, tsHigh);
        if(tmp != HANTRO_OK)
        {
    ...
    case NAL_CODED_SLICE_EXT: {
		...
		pStorage->strm_cavlc = strm;
		pStorage->nalUnit = nalUnit;
		pStorage->cavlc_fill_en = 0;
        tmp = h264bsdDecodeSliceHeader(&strm, pStorage->sliceHeader + 1,
                                       pStorage->activeSps,
                                       pStorage->activePps,
                                       &nalUnit,
                                       pStorage->view ? 1:pStorage->dpb->numRefFrames);
		#if 1
		if((tmp&0xffff) == 0x4567)
		{
			pStorage->cavlc_fill_en = 1;
			VPUMemClean(pDecCont->streamMem);
			pDecCont->hwLength += (((tmp>>16)+7)/8);
			//ALOGV("pDecCont->hwLength %d,  %d", pDecCont->hwLength, (((tmp>>16)+7)/8));
			tmp = HANTRO_OK;
		}
		#endif
        if(tmp != HANTRO_OK)
        ...
        if (H264DEC_SYNC_STREAM == pDecCont->decStat) {
            pStorage->dpb->resync_poc = pStorage->sliceHeader[1].picOrderCntLsb;
            pStorage->dpb->resync_zone = pStorage->activeSps->numRefFrames;
