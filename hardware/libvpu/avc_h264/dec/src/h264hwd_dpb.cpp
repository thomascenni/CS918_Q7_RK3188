...
dpbMem_t* newDpbMem(dpbStorage_t *dpb)
{
    ...
    if (p) {
        ...
        if (dpb->pre_alloc.allocated) {
            memcpy(&p->mem, &dpb->pre_alloc.vpu_mem, sizeof(VPUMemLinear_t));
            p->phy_addr = dpb->pre_alloc.vpu_mem.phy_addr;
            p->vir_addr = (u32)(dpb->pre_alloc.vpu_mem.vir_addr);
            p->memStatus = MEM_AVAIL;
            list_add_tail(&p->list, &dpb->memList);
            dpb->pre_alloc.allocated =0;
            return p;
        } else {
...
#else
        if (0 == VPUMallocLinearFromRender(&p->mem, dpb->picbuffsie,dpb->vpumem_ctx)) {
            p->phy_addr = (u32)p->mem.phy_addr;
            p->vir_addr = (u32)p->mem.vir_addr;
            p->memStatus = MEM_AVAIL;
            list_add_tail(&p->list, &dpb->memList);
...
void *h264bsdAllocateDpbImage(storage_t * pStorage, dpbStorage_t * dpb)
{
    ...
    dpb->vpumem_ctx = pStorage->vpumem_ctx;
...
u32 h264bsdInitDpb(dpbStorage_t * dpb,
                   ...
                   u32 monoChrome, u32 isHighSupported, u32 ts_en, i32 immediate_flag)
{
...
/* Code */
    ...
    dpb->resync_zone = 0;
    dpb->resync_poc  = 0;
    dpb->immediate_flag = immediate_flag;
...
/* Code */
    ...
    return h264bsdInitDpb(dpb, picSizeInMbs, dpbSize, maxRefFrames,
                          maxFrameNum, monoChrome, isHighSupported, ts_en, immediate_flag);
}
...
/* Code */
    ...
    list_for_each_entry(dpbPic, &dpb->dpbList, list) {
        ...
        if (poc == picOrderCnt) {
            if (ts > timeStamp) {
                SMALL_POC_LOG("poc %d == picOrderCnt ts %d > timeStamp %d", poc, ts, timeStamp);
                continue;
            }
        }
        if (poc > picOrderCnt) {
            SMALL_POC_LOG("continue poc %d > picOrderCnt %d", poc, picOrderCnt);
            continue;
        }
...
{
    while((dpb->numOut < targetCount) && (OutputPicture(dpb,false) == HANTRO_OK));
}
