...
void PrintPicBuffer(picBuffer *picBuffer)
{
	...
	ALOGV("Current picture\n");
	ALOGV("Poc i_frame p_frame show ipf grf arf search:\n");
		ALOGV("%-3i %-7i %-7i %-4i %-3i %-3i %-3i %-6i %p %p %p %p\n",
			refPic->poc,
			refPic->i_frame,
			refPic->p_frame,
			refPic->show,
	...
	ALOGV("Ref refPic[picBuffer->size + 1] \n");
	ALOGV("Poc i_frame p_frame show ipf grf arf search:\n");
	for (i = 0; i < picBuffer->size + 1; i++) {
		ALOGV("%-3i %-7i %-7i %-4i %-3i %-3i %-3i %-6i %p %p %p %p\n",
			refPic[i].poc,
	...
	ALOGV("Ref refPicList[picBuffer->size] \n");
	ALOGV("Poc i_frame p_frame show ipf grf arf search:\n");
	for (i = 0; i < picBuffer->size; i++) {
		ALOGV("%-3i %-7i %-7i %-4i %-3i %-3i %-3i %-6i %p %p %p %p\n",
			refPic[i].poc,
