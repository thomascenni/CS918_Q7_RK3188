...
VP8EncRet VP8Init(const VP8EncConfig * pEncCfg, vp8Instance_s ** instAddr)
{
    ...
    *instAddr = NULL;
    ...
    /* Encoder instance */
    inst = (vp8Instance_s *) malloc(1*sizeof(vp8Instance_s));
	...
	memset(inst, 0, sizeof(vp8Instance_s));	//struct initial 0
    ...
    for (i = 0; i < inst->numRefBuffsLum; i++)
        inst->picBuffer.refPic[i].picture.lum = inst->asic.internalImageLuma[i].phy_addr;
    for (i = 0; i < inst->numRefBuffsChr; i++)
        inst->picBuffer.refPic[i].picture.cb = inst->asic.internalImageChroma[i].phy_addr;
...
bool_e SetParameter(vp8Instance_s * inst, const VP8EncConfig * pEncCfg)
{
    ...
    sps->picWidthInPixel    = width;
    sps->picHeightInPixel   = height;
    ...
    VP8_EncSetColorConversion(&inst->preProcess, &inst->asic);
...
{
    /* Check crop */
    if(VP8_EncPreProcessCheck(&inst->preProcess) != ENCHW_OK)
        return ENCHW_NOK;
