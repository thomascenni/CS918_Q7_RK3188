...
{
        entropy_st *entropy = inst->entropy;
...
{
        entropy_st *entropy = inst->entropy;
        u8 *table = (u8 *)inst->asic.cabacCtx.vir_addr;
        i32 i, j, k, l;
...
void UpdateEntropy(vp8Instance_s *inst)
{
	entropy_st *entropy = inst->entropy;
	i32 i, j, k, l, tmp, ii;
	u16 *pCnt = (u16*)inst->asic.probCount.vir_addr;
	u16 *pTmp;
