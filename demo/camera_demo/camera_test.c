...
#else
static ion_device_t *ion_dev = NULL;
static ion_buffer_t *src_buffer = NULL;
#endif
...
int CameraCreate(void)
{
    ...
    if(access(PMEM_DEV_NAME, O_RDWR) < 0) {
            err = ion_open(4096, ION_MODULE_VPU, &ion_dev);
	    ...
	    err = ion_dev->alloc(ion_dev, ALIGN_4K(size), _ION_HEAP_RESERVE, &src_buffer);
	    if (err) {
	    ...
	    m_v4l2Buffer[0] = src_buffer->virt;
	    v4l2Buffer_phy_addr = src_buffer->phys;
...
exit3:
    ion_dev->free(ion_dev, src_buffer);
exit2:
    ion_close(ion_dev);
    ion_dev = NULL;
exit1:
...
//        printf("m_v4l2Buffer[%d]=%p, %d\n", i, m_v4l2Buffer[i], buffer.length);
...
int TaskRuning(int fps_total,int corx,int cory)
{
	...
	while (!isstoped && fps<fps_total)
	{
		...
		printf("fps=%d index=%d buff=%p %d\n", fps, cfilledbuffer1.index, m_v4l2Buffer[cfilledbuffer1.index], cfilledbuffer1.length);
		fwrite(m_v4l2Buffer[cfilledbuffer1.index], 1, frame_size, fp_out);
		if (iDispFd > 0) 
		{	
#if CAM_OVERLAY_BUF_NEW
