...
static RK_S32 parse_options(int argc, char **argv,
                            VpuApiDemoCmdContext_t* cmdCxt)
{
	RK_S8 *opt;
	...
	while (optindex < argc) {
		...
		if (handleoptions && opt[0] == '-' && opt[1] != '\0') {
			...
			case 'o':
				if (argv[optindex]) {
					memcpy(cmdCxt->output_file, argv[optindex],
					       strlen(argv[optindex]));
...
static RK_S32 vpu_encode_demo(VpuApiDemoCmdContext_t *cmd)
{
	...
	*/
	VPU_DEMO_LOG("init vpu api context ok, input yuv stream file size: %d",
	             fileSize);
...
static RK_S32 vpu_decode_demo(VpuApiDemoCmdContext_t *cmd)
{
	...
	*/
	...
	do {
		...
		if ((ret = ctx->decode_sendstream(ctx, pkt)) != 0)
			DECODE_ERR_RET(ERROR_VPU_DECODE);
		else {
			ctx->decode_getframe(ctx, pOut);
...
DECODE_OUT:
	...
	if (ret)
		...
		VPU_DEMO_LOG("decode demo complete OK.");
	return ret;
}
